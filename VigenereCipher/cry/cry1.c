...
void main()
{
    ...
    while (fscanf(fpIn, "%c", &ch) != EOF)
    {
        ...
        {
            printf("%02x", ch); //
            printf(" %02x ", ch ^key[i % KEY_LENGTH]);
            printf("%c", (ch ^key[i % KEY_LENGTH])^key[i % KEY_LENGTH]);
            fprintf(fpOut, "%02X", ch ^ key[i % KEY_LENGTH]);//key[i % KEY_LENGTH]); // ^ is logical XOR
            //printf("%02X", ch ^ key[i % KEY_LENGTH]); // ^ is logical XOR
            i++;
        }
    }
